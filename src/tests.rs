...
fn provider_registry_resolves_or_errors_clearly() {
    ...
    assert!(matches!(worker_from_env("kimi"), Ok(Some(_))), "kimi = its adapter (PHEOBE-8)");
...
fn claude_argv_shape_default_flags_and_cwd() {
    ...
    let argv = f.argv();
    assert_eq!(&argv[..4], &["-p", "the whole prompt", "--output-format", "json"]);
    assert!(
...
fn claude_single_json_with_usage_and_cost() {
    ...
    let out = run_fake(&f, "p").unwrap();
    assert!(out.json_tail.is_some(), "engine JSON handoff must surface as json_tail");
...
fn claude_json_tail_merges_through_run_worker() {
    ...
    set_env("PHEOBE_CLAUDE_BIN", &f.script.display().to_string());
    set_env("PHEOBE_FAKE_ARGV", &f.dir.join("argv.txt").display().to_string());
    let task: Task = serde_json::from_str(r#"{"task":"x","done_when":{"type":"command","run":"true"}}"#).unwrap();
    let out = run_worker(&crate::worker_claude::ClaudeWorker, &task, &f.dir, "claude-tail", "", &[], &LoopCfg::default()).unwrap();
    del_env("PHEOBE_CLAUDE_BIN");
    del_env("PHEOBE_FAKE_ARGV");
    assert!(out.ok);
    assert_eq!(out.summary.as_deref(), Some("did the thing"));
...
fn claude_missing_binary_errors_clearly() {
    ...
    let err = crate::worker_claude::ClaudeWorker.run("p", &dir).unwrap_err();
    del_env("PHEOBE_CLAUDE_BIN");
    let msg = format!("{err:#}");
    assert!(msg.contains("/nonexistent/pheobe-fake-claude"), "got: {msg}");
...
fn claude_nonzero_exit_is_a_clear_error() {
    ...
    assert!(msg.contains("fake claude exploded"), "stderr tail missing: {msg}");
    std::fs::remove_dir_all(&f.dir).ok();
}
...
fn claude_is_error_surfaces_as_failure() {
    ...
    assert!(msg.contains("credit balance too low"), "engine text missing: {msg}");
    std::fs::remove_dir_all(&f.dir).ok();}
...
impl MockOpenAi {
    fn new(bodies: Vec<String>) -> Self {
        ...
        MockOpenAi { base_url: format!("http://{addr}/v1") }
    }
}
...
fn run_task_end_to_end_over_a_mock_endpoint() {
    ...
    let rep = crate::run::run_task(&task, None, &|s| progress.lock().unwrap().push(s.to_string())).unwrap();
    ...
    assert_eq!(
        std::fs::read_to_string(Path::new(&wt).join("src/x.rs")).unwrap().trim(),
        "pub fn x() -> u8 {\n    1\n}"
    );
    // the edit tool's change landed too, and rode the same commit
    assert_eq!(
        std::fs::read_to_string(Path::new(&wt).join("src/lib.rs")).unwrap().trim(),
        "pub fn sprout() {}"
    );
    ...
    assert!(
        progress.lock().unwrap().iter().any(|p| p.contains("🍳 worktree:")),
